...
here::i_am("PopulationDataAmazonianAviary.Rmd")
...
studbook.living <- read.csv(here(paste0("Studbooks/", params$group,"/Studbook_Living.csv")), header = T) %>%
  ...
  select(ID, Sex, Sire, Dam, Birth_Date, Location)  %>%
  mutate(Birth_Date  = str_squish(str_extract(Birth_Date, "(.{1,3}[/-]){2}\\d{1,4}"))) %>%
  mutate(Birth_Date  = str_remove_all(Birth_Date, "\\s")) %>%
  mutate(Birth_Date  = str_replace(Birth_Date, "-(?=0\\d$)", "-20"),
         Birth_Date  = str_replace(Birth_Date, "-(?=[5-9]\\d$)", "-19")) %>%
  mutate(Birth_Date  = if_else(str_detect(Birth_Date, "^\\d{1,}"), 
                                      dmy(Birth_Date), 
                                      mdy(Birth_Date))) %>%
  ...
  group_by(Sire, Dam, Location) %>%
  arrange(Sire, Dam, Location, ID) %>%
  fill(Birth_Date)
```
...
studbook.historic <- read.csv(here(paste0("Studbooks/", params$group,"/Studbook_Historic.csv")), header = T) %>%
  ...
  mutate(Date  = str_squish(str_extract(Date, "(.{1,3}[/-]){2}\\d{1,4}"))) %>%
...
studbook <- births %>%
  ...
  mutate(Last_Location = as.character(label), Last_loc_color = as.character(loc_color)) %>%
  select(-c(label, loc_color)) %>%
  left_join(location.key, by = join_by(Birth_Location == Location)) %>%
  mutate(Birth_Location = as.character(label), Birth_loc_color = as.character(loc_color)) %>%
  select(-c(label, loc_color)) %>%
