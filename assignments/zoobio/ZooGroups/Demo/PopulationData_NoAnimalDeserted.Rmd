...
studbook.living <- read.csv(here(paste0("Studbooks/", params$group,"/Studbook_Living.csv")), header = T) %>%
  ...
  mutate(Date  = str_remove_all(Date, "~ from")) %>%
  mutate(Date  = str_remove_all(Date, "to")) %>%
  mutate(Date  = str_remove_all(Date, "\\+.+$")) %>%
  mutate(Date  = str_remove_all(Date, "(?<=\\d{2}\\s{1}\\d{4})(\\s.+)+")) %>%
...
sb.kin <- studbook %>% 
  mutate(sex_num = if_else(sex_num == 0, 3, sex_num),
         status  = if_else(Status == "Deceased", 1, 0),
         across(c("ID", "Sire", "Dam"), ~as.numeric(.)))
...
```{r}
pedig[["_comp3"]] <- tieLoops(pedig[["_comp3"]])
```


```{r}
pedig[["_comp3"]] <- breakLoops(pedig[["_comp3"]])
