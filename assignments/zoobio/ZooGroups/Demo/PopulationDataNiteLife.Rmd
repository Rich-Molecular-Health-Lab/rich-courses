...
```{r}
studbook.living <- read.csv(here(paste0("Studbooks/", params$group,"/Studbook_Living.csv")), header = T) %>%
  ...
  mutate(across(where(is.character), ~ na_if(., "")),
         ...
         across(c("Sire", "Dam"), ~ str_remove_all(., "(<=?WILD).+")),
         across(c("Sire", "Dam", "ID"), ~ str_remove_all(., "[^\\d+]"))) %>%
...
studbook.historic <- read.csv(here(paste0("Studbooks/", params$group,"/Studbook_Historic.csv")), header = T) %>%
  filter(str_detect(Location, "\\w+")) %>%
  select(ID, Sex, Sire, Dam, Event_Type, Date, Location)  %>%
  mutate(Date  = str_remove_all(Date, ",")) %>%
...
locations <- c(pull(studbook.historic, Location), 
               ...
               pull(transfers.deaths , Last_Location), "UNDETERMINED") %>%
  unique() %>%
  ...
  mutate(label = if_else(row_number(.) > 1 & str_equal(label, lag(label)), str_replace(label, "\\w$", str_end), label)) %>%
  select(label, Location)

location.key <- sample(palettes_d$palettesForR$Cranes, size = length(unique(locations$label)), replace = FALSE) %>%
  as.list() %>%
  ...
  mutate(loc_color = as.character(loc_color))
```
...
if (length(UndetCounts$Birth_Date) > 0) {
  monthly_counts.u <- pmap(UndetCounts, function(Birth_Date, Last_Date, Status)  {
    ...
    tibble(Date = .)  %>%
    group_by(Date) %>%
    summarize(Undetermined = n()) %>% 
    ungroup()
} else {
  monthly_counts.u <- tibble(Date = as.Date(character()), Undetermined = integer())
}
```
...
```{r}
ped.main    <- keep_at(pedig, biggest)
ped.main    <- ped.main[[1]]
famid(ped.main) <- "Main"
...
```{r, out.width="100%", dev='svg'}
plot_pedigrees(ped.main, "Pedigree of Largest Population")
```
...
```{r}
levels <- as.list(generations(ped.main, what = "indiv")) %>%
  set_names(ped.main$ID) %>%
  enframe(name = "id", value = "level") %>%
  mutate(level = as.integer(level)) %>%
  mutate(level = if_else(id %in% founders(ped.main), level, level + 2)) %>%
  mutate(level = if_else(id %in% leaves(  ped.main), level + 1, level))
```


```{r}
levels <- as.list(generations(ped.living, what = "indiv")) %>%
  set_names(ped.living$ID) %>%
  enframe(name = "id", value = "level") %>%
  mutate(level = as.integer(level)) %>%
...
offspring.edf <- list_merge(sire, !!!dam) %>%
  imap(\(x, idx) list(idx, as.list(commonDescendants(ped.living, ids = x, maxGen = 2)))) %>%
...
nodes <- as.data.frame(ped.living) %>% 
  ...
  mutate(icon = map2(color, code, \(x, y) list(code = y, color = x, face = "'Font Awesome 5 Free'", weight = 700, size = 30)),
         font = map(color, \(x) list(size = 12, color = x, strokeColor = "#FFFFFF", strokeWidth = 3))
         ) %>%
  select(id, level, group, shape, icon, label, font) %>%
  bind_rows(pairs.ndf) %>% distinct()
```
