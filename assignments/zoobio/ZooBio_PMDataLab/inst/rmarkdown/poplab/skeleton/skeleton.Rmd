...
# Makeup Version of the Population Data Management Lab

>The code below should all run without any problem once you have installed the necessary packages. You will analyze the studbook for the Pygmy Slow Loris Provisional SSP. Examine each of the tables and figures created and answer the prompts to produce a report for submission.
...
source(system.file("R", "conflicts.R" , package = "zooBioTools" ))
source(system.file("R", "setup.R"     , package = "zooBioTools" ))
source(system.file("R", "functions.R" , package = "zooBioTools" ))
source(system.file("R", "plots.R"     , package = "zooBioTools" ))
source(system.file("R", "reactables.R", package = "zooBioTools" ))
```
...
```{r}
studbook <- read_tsv(system.file("extdata", "studbook.tsv", package = "zooBioTools")) %>%
  mutate(across(c(DateBirth, DateDeath),  ~ ymd(.)))
timeline <- read_tsv(system.file("extdata", "timeline.tsv", package = "zooBioTools")) %>%
...
```{r, out.width="100%"}
studbook.tbl <- studbook %>% studbook.react(., studbook.cols(.), groupBy = "Status")

save_reactable_test(studbook.tbl, "StudbookA.html")
studbook.tbl
```
...
population.plot <- plot_census(raw.counts, 
                               colors, 
                               "Pygmy Loris Population Trends 1966-2025")
population.plot
```
...
lambda.plot  <- plot_lambda(lifeTab.5yrs)
lambda.plot
```
...
founder.vis <- founder.summary %>% studbook.react(., cols = founder.cols(.))
...
living.summary <- family_history(ped.living, studbook) %>% 
  studbook.react(., cols = living.cols(.), columnGroups = living.groups)
...
save_reactable_test(kinship.summary, paste0("kin_", params$group, ".html"))
kinship.summary
```
