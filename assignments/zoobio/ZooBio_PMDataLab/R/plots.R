...
lighten_plotly_pal <- function(palette, hex) {
  fills   <- gsub("FF", hex, palette)
  return(fills)
}

lighten_pal_list <- function(palette, hex) {
  fill.col <- map_depth(palette, 1, \(x) gsub("FF", hex, x))
  return(fill.col)
}
...
plot_lambda <- function(life.table) {

  col.pal <- set_plotly_pal(colors)
  fills   <- lighten_plotly_pal(col.pal, "33")

  life.table.static <- lifeTab_static(life.table) %>% annotate_lambda()
...
plot_census <- function(cesus.df, palette, title) {
