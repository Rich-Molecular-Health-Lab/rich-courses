...
`r newthought(By the end of this activity)`, you should be able to:

1.  `r newthought(Explain)` the concepts of carrying **capacity and maximum sustainable yield (MSY)**.

2.  `r newthought(Compare)` different management strategies—Open Access, Fixed Quota, and Fixed Effort—and their outcomes for resource sustainability.

3.  `r newthought(Analyze)` how cooperative vs. individual incentives influence resource use decisions (the *Tragedy of the Commons*).

4.  `r newthought(Reflect)` on real-world implications of fisheries management strategies and how they might be adapted to other renewable resources.

# Background & Context

`r newthought(In natural resource management)`, fisheries provide a classic example of how unregulated harvesting can lead to **resource depletion**—sometimes rapidly and unexpectedly. This phenomenon is known as the **Tragedy of the Commons**, where individuals, acting in their own self-interest, collectively over-exploit a shared resource.
...
`r newthought(Before moving on)`, think about the **maximum number of fish** the population can yield each round without declining over time. This is the **Maximum Sustainable Yield (MSY)**. Because our population doubles each round (with a cap at 30), the MSY will be near the midpoint of carrying capacity. If you remove too many, you risk dropping below the replenishment threshold; if you remove too few, you might not maximize yields.
